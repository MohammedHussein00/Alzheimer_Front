...
@Component({
  selector: 'app-dash-doctors',
  templateUrl: './dash-doctors.component.html',
  styleUrls: ['./dash-doctors.component.css']
})
export class DashDoctorsComponent {
  clearForm!: FormGroup;
  ...
  ngOnInit() {
    ...
    this.GetDoctors().subscribe(data => {
      ...
      this.tempDoctors = data;
      this.tempDoctors.sort((a, b) => b.specialization.localeCompare(a.specialization));
...
if (this.tempDoctors) {
  this.tempDoctors = this.tempDoctors.map(d => {
    ...
    if (d && d.certification) {
      d.certification = d.certification.replace('E:\\Angular Projects\\GradProject\\Alzheimer\\DoctorData\\', 'DoctorData\\').replace(/\\/g, '/');
    }
    return d;
  });
} else {
...
if (this.doctors) {
  ...
  GetDoctors(): Observable<Doctorinfo[]> {
    return this.http.get<Doctorinfo[]>('https://localhost:7213/Admin/get-doctors');
  }
